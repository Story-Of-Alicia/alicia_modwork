...
Resource ReadResource(uint8_t *bytes) {
...
int main() {
    ...
    for (; currentOffset < RES_OFFSET; currentOffset++) {
        ...
        if (bufferIndex == 620) {
            if (buffer[0] != 0) {
                Resource resource = ReadResource(buffer);
                fprintf(log, "Offset: 0x%-10X Length: 0x%-10X ", resource.resourceOffset, resource.GetLength());
                fprintf(log, "Path: %ls\n", resource.path);
                entryCount++;
            }
            bufferIndex = 0;
