...
{
  if (!ReadProcessMemory(procHandle, (LPCVOID) addr, (LPVOID) buffer.dataPtr(), (SIZE_T) buffer.getSize(), nullptr))
  {
    return GetLastError();
  }
  return 0;
...
{
  if (!WriteProcessMemory(procHandle, (LPVOID) addr, (LPCVOID) buffer.dataPtr(), (SIZE_T) buffer.getSize(), nullptr))
  {
    return GetLastError();
  }
  return 0;
...
void dbg::DebuggingTool::renderInterface()
{
  ...
  if (this->m_ui_showMemory)
  {
    ...
    if (ImGui::Begin(fmt::format("(Debugger Tool) Memory - {}", this->m_processName).c_str(), nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse))
    {
      // Opts
      {
      ...
      {
        char addrStr[12] = {0};
        if (ImGui::InputText("Address", addrStr, 12, ImGuiInputTextFlags_EnterReturnsTrue | ImGuiInputTextFlags_CharsHexadecimal))
        {
          try
          {
        ...
        ImGui::Spacing();
        int32_t scroll = 0;
        ImGui::VSliderInt("", {10, ImGui::GetWindowHeight() - 80}, &scroll, -12, 12, "", 0);
        this->m_memoryPtr -= scroll * 8;
      }

      ImGui::SameLine();
      ImGui::BeginChild("memory_view_area", {0, 0}, false, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);
